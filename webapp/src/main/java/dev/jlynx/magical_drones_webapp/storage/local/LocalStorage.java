package dev.jlynx.magical_drones_webapp.storage.local;
...
public class LocalStorage implements StorageService {
     ...
     * @throws StorageException if the path doesn't exist or is not a file or if an I/O error occurs
     * while reading file contents
     */
    @Override
    public byte[] download(String bucketName, String key) throws StorageException {
        Path path = Paths.get(bucketName, key);
        if (!path.toFile().isFile()) {
            log.debug("Tried to access path '{}' which is not a file or doesn't exist", path);
            throw new StorageException("Path '%s' is not a file or doesn't exist.".formatted(path.toString()));
        }
